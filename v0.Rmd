...
p1<-plot_ly(diam_mid_chl_small_table, y= ~chl_small,x = ~diam_mid, frame = ~times, size = ~counts,type = 'scatter', mode = 'markers',sizes = c(0,2000), opacity = 0.5,alpha = 0.3,showlegend = FALSE)
p2<-plot_ly(pe_chl_small_table, y = ~pe, x = ~chl_small, frame = ~times, size = ~ counts,type = 'scatter', mode = 'markers',alpha = 0.3, opacity=0.5,sizes = c(0,2000),howlegend = FALSE)
p3<-plot_ly(diam_mid_pe_table, x= ~pe,y = ~diam_mid, frame = ~times,size = ~ counts,type = 'scatter', mode = 'markers',opacity=0.5,sizes = c(0,2000),alpha = 0.3, showlegend = FALSE)
p <- subplot(p1,p2,p3)
...
pie_table_large
prob <- highlight_key(pie_table_large, ~Cluster)
p4 <- plot_ly(prob, frame = ~time,tickformat = '%m-%d') %>%
  group_by(Cluster) %>%
  add_lines(x = ~Time, y = ~Prob, color = ~Cluster)%>%
  add_segments(x = ~time,xend = ~time, y = 0,yend = 1,frame = ~time, showlegend = FALSE)%>%
